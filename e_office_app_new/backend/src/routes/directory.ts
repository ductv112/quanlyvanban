...
router.get('/', async (req: Request, res: Response) => {
  try {
    // Enforce unitId from JWT for multi-tenancy by default
    const { unitId } = (req as AuthRequest).user;
    const { unit_id, department_id, search, page, page_size } = req.query;

    // Use query param unit_id if provided, else fall back to JWT unitId
    const targetUnitId = unit_id ? Number(unit_id) : unitId;
