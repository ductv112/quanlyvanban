...
function canSignDoc(
  ...
  // Fallback: legacy data (chi co text signer/approver, khong co ID)
...
export default function OutgoingDocDetailPage() {
  ...
  // Nếu attachment đã is_ca=true -> không cho ký lại (PAdES rule, sẽ invalidate signature cũ).
  // Nếu attachment chưa ký (dự thảo phát hành chưa ký, hoặc upload thêm sau phát hành) -> cho ký.
  ...
  return (
    <div>
      ...
      <Drawer
        ...
        extra={<Space><Button onClick={() => setGiaoViecOpen(false)} ghost style={{ borderColor: 'rgba(255,255,255,0.6)', color: '#fff' }}>Hủy</Button><Button type="primary" loading={giaoViecSaving} onClick={handleGiaoViec}>Tạo và giao việc</Button></Space>} mask={{ closable: false }}>
      ...
      <Drawer
        ...
        extra={<Space><Button onClick={() => setArchiveOpen(false)}>Hủy</Button><Button type="primary" onClick={handleArchive} loading={archiving}>Chuyển lưu trữ</Button></Space>} mask={{ closable: false }}>
      ...
      {/* Sign modal từ useSigning hook — ký số khi VB đi attachment chưa kế thừa CK từ dự thảo */}
      {renderSignModal()}
