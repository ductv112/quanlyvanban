...
Tài liệu này mô tả đầy đủ cách sử dụng tính năng **Thông báo** của hệ thống Quản lý văn bản điện tử (e-Office), bao gồm hai nơi truy cập: **chuông thông báo trên thanh đầu trang** và **trang Thông báo** (đường dẫn `/thong-bao`).

---

## 1. Giới thiệu

Trong quá trình làm việc, mỗi cán bộ thường nhận được nhiều loại thông tin từ hệ thống: kết quả ký số, các thông báo nội bộ do quản trị viên gửi đến đơn vị… Để tránh phải mở từng màn hình kiểm tra, hệ thống tập trung tất cả các loại thông báo này vào một chỗ và **báo ngay cho người dùng** bằng huy hiệu số trên biểu tượng chuông ở góc trên bên phải.

Tính năng **Thông báo** có hai nơi truy cập, hoạt động bổ sung cho nhau:

- **Chuông thông báo trên thanh đầu trang** — biểu tượng chuông kèm số đếm chưa đọc, có ở mọi màn hình. Mở ra danh sách rút gọn (10 thông báo gần nhất) để xem nhanh.
- **Trang Thông báo** (`/thong-bao`) — danh sách đầy đủ, có phân trang và lọc theo trạng thái đã đọc / chưa đọc; truy cập từ menu bên trái mục **Thông báo**.
...
|---|---|---|---|
| **Ký số thành công** (`sign_completed`) | Chuông thông báo trên thanh đầu trang | Sau khi giao dịch ký số của chính người dùng kết thúc thành công (do hệ thống ký số nền — `signing-poll.worker`) | **Danh sách giao dịch ký số** → tab **Đã ký**. Đường dẫn: `/ky-so/danh-sach?tab=completed` |
| **Ký số thất bại / hết hạn / đã hủy** (`sign_failed`) | Chuông thông báo trên thanh đầu trang | Khi giao dịch ký số của chính người dùng bị **thất bại**, **hết hạn** (không xác nhận trong 3 phút) hoặc **bị hủy** | **Danh sách giao dịch ký số** → tab **Thất bại**. Đường dẫn: `/ky-so/danh-sach?tab=failed` |
...
> **Phạm vi thông báo qua chuông tại phiên bản hiện tại**: chuông trên thanh đầu trang **chỉ phát thông báo cá nhân cho hai sự kiện liên quan tới ký số** — *Ký số thành công* và *Ký số thất bại / hết hạn / đã hủy*. Các sự kiện nghiệp vụ khác (giao việc, có văn bản đến mới, được phân công xử lý, ý kiến chỉ đạo, thay đổi trạng thái hồ sơ công việc…) **chưa được nối vào chuông** ở phiên bản này — nếu cần biết, người dùng vẫn phải vào thẳng các màn hình tương ứng (Văn bản đến, Bút phê, Hồ sơ công việc…) để theo dõi.
